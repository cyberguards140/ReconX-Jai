...
from core.finding_db import SessionLocal as FindingSession, Finding, RiskScore
from core.web_db import SessionLocal as WebSession, WebAsset, Secret, Technology
from core.cloud_db import SessionLocal as CloudSession, CloudAsset, StorageBucket, IAMEntity, PublicExposure, CloudRelationship
from core.project_db import SessionLocal as ProjectSession, Project, Target, Campaign, ScanProfile, Note
from core.report_db import SessionLocal as ReportSession, Report as ReportDB, ReportHistory
from core.automation_db import SessionLocal as AutoSession, Workflow, ScheduledJob, TaskQueue, Alert, WorkflowStep
from core.intelligence_db import SessionLocal as IntelSession, UniversalAsset, Relationship, AssetTimeline
from core.screenshot_db import SessionLocal as ScreenSession, Screenshot, VisualFinding, OCRResult, VisualChange
from core.threat_db import SessionLocal as ThreatSession, CVEData, IOCData, ReputationData, PassiveDNS, ASNData, ExposureEvent
from core.auth_db import SessionLocal as AuthSession, User, Role, Organization, Team, AuditLog, ActivityLog, ApprovalRequest
from core.plugin_db import SessionLocal as PluginSession, Plugin, MarketplacePackage, ConnectorRegistry, ThemeRegistry
from core.distributed_db import SessionLocal as DistSession, Node, Agent, Cluster, DistributedJob, NodeHealth
from core.analytics_db import SessionLocal as AnalyticsSession, HistoricalSnapshot, TrendData, ExecutiveMetric, ForecastData, ExposureMetric, RiskMetric
from core.graph_db import SessionLocal as GraphSession, GraphEntity, GraphRelationship, AttackPath, Investigation, InvestigationNote
from core.exposure_db import SessionLocal as ExposureSession, Exposure, RiskScore, SecurityPosture, RemediationAction, Ownership, SLAPolicy, RiskReduction
from core.campaign_db import SessionLocal as CampaignSession, Campaign, Engagement, AssessmentScope, CampaignTask, CampaignMilestone, Evidence
from core.case_db import SessionLocal as CaseSession, Case, Investigation as CaseInvestigation, Artifact, CaseReview
...
def get_assets():
    session = SessionManager()
    if not session.current_project: return jsonify([])
...
def get_web_assets(asset_type):
    ...
    # normalize type
    if asset_type == "javascript": t = "js_file"
    elif asset_type == "parameters": t = "parameter"
    elif asset_type == "endpoints": t = "endpoint"
    else: t = "url"
    
    session = SessionManager()
    if not session.current_project: return jsonify([])
    db = WebSession()
    assets = db.query(WebAsset).filter(WebAsset.project_id == session.current_project, WebAsset.asset_type == t).all()
...
def get_cloud_assets():
    ...
    res = [{"id": a.id, "provider": a.provider, "name": a.name, "resource_type": a.resource_type, "public": a.public} for a in assets]
    db.close()
    return jsonify(res)
...
def get_cloud_storage():
    ...
    if not session.current_project: return jsonify([])
    db = CloudSession()
    buckets = db.query(StorageBucket).filter(StorageBucket.project_id == session.current_project).all()
    res = [{"id": b.id, "provider": b.provider, "name": b.name, "public": b.public, "object_count": b.object_count} for b in buckets]
    db.close()
    return jsonify(res)
...
def get_cloud_iam():
    ...
    res = [{"id": e.id, "provider": e.provider, "entity_type": e.entity_type, "name": e.name} for e in entities]
    db.close()
    return jsonify(res)
...
def get_cloud_exposures():
    ...
    if not session.current_project: return jsonify([])
    db = CloudSession()
    exps = db.query(PublicExposure).filter(PublicExposure.project_id == session.current_project).all()
    res = [{"id": e.id, "asset_id": e.asset_id, "finding": e.finding, "severity": e.severity} for e in exps]
    db.close()
    return jsonify(res)
...
def get_projects():
    ...
    res = [{"id": p.id, "name": p.name, "description": p.description, "status": p.status} for p in projects]
    db.close()
    return jsonify(res)
...
def get_reports():
    ...
    res = [{"id": r.id, "type": r.report_type, "version": r.version, "path": r.file_path} for r in reps]
    db.close()
    return jsonify(res)
...
def get_asset_timeline(id):
    db = IntelSession()
    events = db.query(AssetTimeline).filter(AssetTimeline.asset_id == id).order_by(AssetTimeline.timestamp.desc()).all()
...
def get_reputation():
    ...
    res = [{"id": r.id, "asset_id": r.asset_id, "status": r.status, "score": r.reputation_score} for r in reps]
    db.close()
    return jsonify(res)
...
def auth_login():
    ...
    from auth.authentication import AuthenticationEngine
...
def get_marketplace():
    from plugins.marketplace_engine import MarketplaceEngine
...
def get_analytics_metrics():
    ...
    res = [{"id": s.id, "assets": s.total_assets, "findings": s.total_findings, "date": s.snapshot_date} for s in snaps]
    db.close()
    return jsonify(res)
...
def get_analytics_trends():
    ...
    res = [{"id": t.id, "category": t.trend_category, "delta": t.delta_value, "desc": t.description} for t in trends]
    db.close()
    return jsonify(res)
...
def get_analytics_risk():
    ...
    res = [{"id": r.id, "project": r.project_id, "score": r.risk_score, "date": r.timestamp} for r in risks]
    db.close()
    return jsonify(res)
...
def get_analytics_exposure():
    ...
    res = [{"id": e.id, "project": e.project_id, "score": e.exposure_score, "date": e.timestamp} for e in exposures]
    db.close()
    return jsonify(res)
...
def get_analytics_forecast():
    ...
    res = [{"id": f.id, "category": f.forecast_category, "target": f.projected_value, "date": f.target_date} for f in forecasts]
    db.close()
    return jsonify(res)
...
def get_graph_relationships():
    ...
    res = [{"id": r.id, "source": r.source_id, "target": r.target_id, "type": r.relation_type, "confidence": r.confidence_score} for r in rels]
    db.close()
    return jsonify(res)
...
def get_exposures():
    ...
    res = [{"id": e.id, "asset": e.asset_id, "type": e.exposure_type, "severity": e.severity, "status": e.status} for e in exps]
    db.close()
    return jsonify(res)
...
def get_security_posture():
    ...
    res = [{"id": p.id, "project": p.project_id, "score": p.score, "grade": p.grade} for p in postures]
    db.close()
    return jsonify(res)
...
def manage_campaigns_api():
    if request.method == 'POST':
        ...
        c_id = CampaignManager.create_campaign(data.get("name", "New Campaign"), data.get("type", "External ASM"))
        return jsonify({"status": "created", "campaign_id": c_id})
    else:
        db = CampaignSession()
        camps = db.query(Campaign).all()
        res = [{"id": c.id, "name": c.name, "type": c.campaign_type, "status": c.status} for c in camps]
        db.close()
        return jsonify(res)
...
def get_engagements():
    ...
    res = [{"id": e.id, "name": e.engagement_name, "client": e.client, "status": e.status} for e in engs]
    db.close()
    return jsonify(res)
...
def get_scopes():
    ...
    res = [{"id": s.id, "campaign": s.campaign_id, "target": s.target, "status": s.status} for s in scopes]
    db.close()
    return jsonify(res)
...
def get_tasks():
    ...
    res = [{"id": t.id, "name": t.task_name, "status": t.status, "assignee": t.assigned_to} for t in tasks]
    db.close()
    return jsonify(res)
...
def manage_cases_api():
    if request.method == 'POST':
        ...
        c_id = CaseManager.create_case(data.get("title", "New Case"), data.get("type", "Threat Investigation"))
        return jsonify({"status": "created", "case_id": c_id})
    else:
        db = CaseSession()
        cases = db.query(Case).all()
        res = [{"id": c.id, "title": c.title, "type": c.case_type, "status": c.status} for c in cases]
        db.close()
        return jsonify(res)
...
def get_case_investigations():
    ...
    res = [{"id": i.id, "case": i.case_id, "name": i.investigation_name, "owner": i.owner} for i in invs]
    db.close()
    return jsonify(res)
...
def get_artifacts():
    ...
    res = [{"id": a.id, "name": a.artifact_name, "type": a.artifact_type, "path": a.file_path} for a in arts]
    db.close()
    return jsonify(res)
...
def get_reviews():
    ...
    res = [{"id": r.id, "case": r.case_id, "reviewer": r.reviewer, "status": r.status} for r in revs]
    db.close()
    return jsonify(res)
