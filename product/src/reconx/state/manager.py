...
class StateManager:
    ...
    async def create_execution(
        ...
        exec_record = await workflow_execution_repo.create(self.db, obj_in={
            "workflow_name": name,
            "target": target,
            "user_id": user_id,
    ...
    async def update_status(
        ...
        if record:
            ...
            if status in [WorkflowState.COMPLETED.value, WorkflowState.FAILED.value, WorkflowState.CANCELLED.value]:
    ...
    async def get_execution(self, exec_id: str) -> Optional[DBWorkflowExecution]:
