...
import logging
from reconx.modules.asm_core.ingest_engine import IngestEngine
from reconx.modules.asm_core.validation import AbstractValidationLayer
from reconx.modules.asm_core.enrichment import EnrichmentEngine
from reconx.modules.asm_core.correlation import CorrelationEngine
...
class ASMOrchestrator:
    ...
    def process_records(self, records: List[Dict[str, Any]], source: str) -> Dict[str, Any]:
        ...
        logger.info(f"Processed {len(records)} records from {source}. Yielded {len(assets)} normalized assets.")
        return results
