...
from reconx.modules.infra_intel.schema import InfraEntityModel, ASNNode, NetblockNode, OrganizationNode, CertificateNode
            ...
            if node.entity_type == "netblock" and cached_node.entity_type == "asn":
                if node.owner_asn == cached_node.value:
                    edges.append({
                        "source": cached_id,
                        "target": node.id,
                        "relationship": "ANNOUNCES"
                    })
                    
            # Organization -> ASN/Netblock = OWNS
            if node.entity_type in ("asn", "netblock") and cached_node.entity_type == "organization":
