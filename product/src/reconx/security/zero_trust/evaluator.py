...
class ZeroTrustEvaluator:
    ...
    async def evaluate_request(self, request: Request, user_metadata: Optional[Dict[str, Any]] = None):
        """
        ...
        if ip in self.banned_ips:
            ...
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Access denied by zero-trust policy (IP)")
        ...
        if current_risk_score >= self.max_risk_tolerance:
            logger.warning(f"Zero Trust blocked request due to high risk score: {current_risk_score}")
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Session context risk too high. Re-authentication required.")
