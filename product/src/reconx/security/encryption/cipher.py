...
class EncryptionManager:
    ...
    def __init__(self):
        ...
        self.master_key_b64 = os.getenv("ENCRYPTION_MASTER_KEY", "uE4F7jC9gLzRkXbQyNpVt3H+V8s2M6wY0zF/K5Xq/8Y=")
        self.key = base64.b64decode(self.master_key_b64)
        
        if len(self.key) != 32:
            logger.warning("ENCRYPTION_MASTER_KEY is not 32 bytes. Encryption may fail or be insecure.")
    ...
    def decrypt(self, encoded_payload: str, associated_data: bytes = None) -> str:
        ...
        try:
            ...
            logger.error("Decryption failed. The data may be tampered with or the wrong key/AAD was used.")
