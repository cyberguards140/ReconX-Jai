...
from reconx.database.models import User, ReportRecord, ReportSchedule
...
class GenerateReportRequest(BaseModel):
    ...
    distribute_to: Optional[List[dict]] = None
    report_data: Optional[dict] = None # For injecting specific data
...
@router.post("/generate", response_model=dict, dependencies=[Depends(require_permission("report.generate"))])
async def generate_report(
    ...
    if request.distribute_to:
        background_tasks.add_task(ReportDistributor.distribute, record.report_id, request.distribute_to)

    return {"status": "success", "report_id": record.report_id, "file_path": record.metadata_json.get("file_path")}

@router.get("/", response_model=List[dict], dependencies=[Depends(require_permission("report.read"))])
...
@router.post("/schedules", response_model=dict, dependencies=[Depends(require_permission("report.generate"))])
...
@router.get("/schedules", response_model=List[dict], dependencies=[Depends(require_permission("report.read"))])
