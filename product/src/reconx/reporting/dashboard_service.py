...
from reconx.database.repositories.asset import asset_repo
from reconx.database.repositories.finding import finding_repo
from reconx.database.models import Port, Service, Route, DNSRecord, AssetRelationship, Technology, CMS, Framework, WAF, ContentItem, Endpoint, VirtualHost, Certificate, TLSConfiguration, CipherSuite, HTTPHeader, SMBHost, SMBShare, SMBUser, SMBGroup, SMBPasswordPolicy, SMBConfiguration, ADDomain, ADUser, ADGroup, ADComputer, ADTrustRelationship, OSINTOrganization, OSINTEmail, OSINTUsername, OSINTProfile, OSINTRelationship, TrafficFlow, TrafficProtocol, TrafficCommunication, TrafficDNSQuery, TrafficHTTPMetadata, TrafficTLSMetadata, TrafficStatistic, CloudAccount, CloudResource, CloudKubernetesCluster, CloudNamespace, CloudPod, CloudDeployment, CloudContainerImage, CloudContainerFinding, VulnCVE, VulnRecord, VulnRiskProfile, ReportRecord, ReportEvidence, GraphNode, GraphEdge, AnalyticsRiskScore, AnalyticsAssetProfile, AnalyticsRecommendation, AnalyticsTrendData, AnalyticsPriority, AutomationJob, AutomationAlert, AutomationChangeEvent, EnterpriseTenant, EnterpriseUser, EnterpriseAPIKey, EnterpriseAuditLog
from reconx.services.intelligence.risk_scoring import RiskScoring
...
class DashboardService:
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        vuln_prof_res = await self.db.execute(select(VulnRiskProfile).order_by(VulnRiskProfile.risk_score.desc()).limit(10))
        ...
        critical_count = len([f for f in f_dicts if f["severity"] == "CRITICAL" or f["severity"] == "critical"])
        high_count = len([f for f in f_dicts if f["severity"] == "HIGH" or f["severity"] == "high"])
        medium_count = len([f for f in f_dicts if f["severity"] == "MEDIUM" or f["severity"] == "medium"])
        ...
        top_services = dict(sorted(service_counts.items(), key=lambda item: item[1], reverse=True)[:5])

        # Tech Aggregations
        tech_counts = {}
        for t in technologies:
            tech_counts[t.technology] = tech_counts.get(t.technology, 0) + 1
        top_technologies = dict(sorted(tech_counts.items(), key=lambda item: item[1], reverse=True)[:5])
        ...
        top_sources = dict(sorted(source_counts.items(), key=lambda item: item[1], reverse=True)[:5])
        ...
        analytics_profiles = (await self.db.execute(select(AnalyticsAssetProfile).order_by(AnalyticsAssetProfile.risk_score.desc()).limit(10))).scalars().all()

        # Automation Aggregation
        pending_jobs = len((await self.db.execute(select(AutomationJob).where(AutomationJob.status == "Pending"))).scalars().all())
        recent_alerts = (await self.db.execute(select(AutomationAlert).order_by(AutomationAlert.timestamp.desc()).limit(5))).scalars().all()
        recent_changes = (await self.db.execute(select(AutomationChangeEvent).order_by(AutomationChangeEvent.timestamp.desc()).limit(5))).scalars().all()
        ...
        api_keys_active = len((await self.db.execute(select(EnterpriseAPIKey).where(EnterpriseAPIKey.status == "Active"))).scalars().all())
        recent_audits = (await self.db.execute(select(EnterpriseAuditLog).order_by(EnterpriseAuditLog.timestamp.desc()).limit(5))).scalars().all()
        ...
        return {
            ...
            "vulnerability_overview": {
                ...
                "highest_risk_assets": [{"asset_id": p.asset_id, "score": p.risk_score} for p in top_risk_profiles]
            },
            "dns_stats": dns_counts,
            "relationships_mapped": len(relationships),
            "risk_score": risk_score,
            "reports": {
                "generated": [r.title for r in (await self.db.execute(select(ReportRecord))).scalars().all()],
                "evidence_files": len((await self.db.execute(select(ReportEvidence))).scalars().all())
            },
            "graph_intelligence": {
            ...
            "analytics": {
                "top_recommendations": [{"title": r.title, "impact": r.impact} for r in analytics_recs],
                "top_assets": [{"asset_id": p.asset_id, "score": p.risk_score} for p in analytics_profiles]
            },
            "automation": {
                "pending_jobs": pending_jobs,
                "recent_alerts": [{"message": a.message, "severity": a.severity} for a in recent_alerts],
                "recent_changes": [{"type": c.change_type, "entity": c.entity_type} for c in recent_changes]
            },
            "enterprise": {
                "total_tenants": tenant_count,
                "total_users": user_count,
                "active_api_keys": api_keys_active,
                "recent_audits": [{"user_id": a.user_id, "action": a.action} for a in recent_audits]
            },
    ...
    async def get_attack_surface_graph(self) -> dict:
        ...
        nodes_list = [{"node_id": n.node_id, "node_type": n.node_type, "label": n.label} for n in nodes]
        edges_list = [{"source": e.source_node_id, "target": e.target_node_id, "type": e.edge_type} for e in edges]
