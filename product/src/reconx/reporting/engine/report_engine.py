...
from reconx.database.models import ReportRecord
from reconx.reporting.engine.renderer import get_renderer
from reconx.reporting.engine.exporter import DataExporter
...
class ReportEngine:
    ...
    async def generate_executive_report(
        ...
        context = {
            ...
            "summary": report_data.get("summary", "This report provides an executive overview of the security posture for the specified scope."),
            ...
            "recommendations": report_data.get("recommendations", ["Implement strict access controls", "Patch critical vulnerabilities"])
        } if report_data else {
            "title": title,
            "generated_at": datetime.now(timezone.utc),
            "target_scope": target_scope,
        ...
        record = ReportRecord(
            ...
            metadata_json={"target_scope": target_scope, "format": export_format, "file_path": file_path}
        )
        session.add(record)
