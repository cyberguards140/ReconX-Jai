...
from reconx.workflow.task import Task, TaskStatus
...
from reconx.plugins.base import PluginResult
...
async def test_scheduler_timeout():
    ...
    with patch("reconx.workflow.scheduler.plugin_manager.execute_plugin", new=mock_execute):
        # We also need to mock async_session_factory
        class MockDB:
            async def __aenter__(self): return self
            async def __aexit__(self, *args): pass
            
        with patch("reconx.workflow.scheduler.async_session_factory", return_value=MockDB()):
            await scheduler.run()
