...
class ComplianceMapper:
    ...
    def __init__(self):
        ...
        self.matrices = {
            "ISO27001": {
                ...
                "A.13.1.1": {
                    "description": "Network Controls (Exposure)",
                    "triggers": ["critical_exposure"],
                },
                "A.14.2.5": {
    ...
    def generate_gap_report(
        self, framework: str, findings: list[dict[str, Any]], exposures: list[dict[str, Any]]
    ) -> str:
        """
        ...
        for control, details in matrix.items():
            ...
            for finding in findings:
                if finding.get("severity", "").lower() in details["triggers"]:
                    report += (
                        f"- ❌ **Gap**: {finding.get('title')} on `{finding.get('asset_target')}`\n"
                    )
                    control_failures += 1
