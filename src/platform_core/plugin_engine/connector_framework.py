...
from core.legacy_core.plugin_db import ConnectorRegistry, SessionLocal
...
class ConnectorFramework:
    ...
    def register_connector(connector_name, category, config_dict):
        db = SessionLocal()
        c = (
            db.query(ConnectorRegistry)
            .filter(ConnectorRegistry.connector_name == connector_name)
            .first()
        )
        if not c:
            c = ConnectorRegistry(
    ...
    def dispatch_payload(connector_name, payload):
        ...
        c = (
            db.query(ConnectorRegistry)
            .filter(
                ConnectorRegistry.connector_name == connector_name,
