...
from data.database.repositories.asset import asset_history_repo, asset_rel_repo, asset_repo
from recon.services.intelligence.asset_correlator import AssetCorrelator
from recon.services.intelligence.deduplicator import Deduplicator
from recon.services.intelligence.relationship_engine import RelationshipEngine
from recon.services.intelligence.schemas import AssetSchema
...
class IntelligenceStore:
    ...
    async def ingest_plugin_result(self, project_id: str, results: dict[str, Any]):
        ...
        for a in deduped_assets:
            # check if exists
            existing = await asset_repo.get_by_value(
                self.db, project_id, a["asset_type"], a["value"]
            )
            if not existing:
                ...
                await asset_history_repo.create(
                    self.db, obj_in={"asset_id": new_asset.id, "event": "Discovered"}
                )
            else:
                asset_id_map[a["value"]] = existing.id
                await asset_history_repo.create(
                    self.db,
                    obj_in={"asset_id": existing.id, "event": f"Seen again via {a['source']}"},
                )
    ...
    async def get_assets(self) -> list[dict[str, Any]]:
        ...
        return [
            {"id": a.id, "type": a.asset_type, "value": a.value, "source": a.source} for a in assets
        ]
    ...
    async def get_asset_graph(self) -> dict[str, Any]:
        ...
        graph = {
            "nodes": [
                {"id": a.id, "value": a.value, "type": a.asset_type} for a in assets.values()
            ],
            "edges": [
                {
