...
from core.auth.audit_identity import log_security_event
from core.auth.jwt_manager import create_access_token, create_refresh_token
from core.auth.session_manager import create_session
from data.database.models import User
from data.database.repositories.login_attempt import login_attempt_repo
from data.database.repositories.password_history import password_history_repo
from data.database.repositories.user import user_repo
from recon.services.auth.password_manager import (
    hash_password,
    validate_password_strength,
    verify_password,
)
from recon.services.auth.refresh_tokens import store_refresh_token
...
async def register_user(
    ...
    # Check if exists
    existing_user = await user_repo.get_by_username_or_email(db, username, email)
    if existing_user:
        raise HTTPException(
...
async def authenticate_user(
    ...
    attempts = await login_attempt_repo.get_recent_failures(db, username, lockout_threshold)
    if len(attempts) >= MAX_LOGIN_ATTEMPTS:
        await log_security_event(
            db, action="Account Lockout", resource_id=username, ip_address=ip_address
        )
        raise HTTPException(
    ...
    await log_security_event(db, action="Login", user_id=user.id, ip_address=ip_address)
...
async def change_password(
    ...
    # Check history
    history = await password_history_repo.get_recent_history(db, user_id, PASSWORD_HISTORY_LIMIT)
    for hist in history:
