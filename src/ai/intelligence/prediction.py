    ...
    def forecast_risk(self, asset: dict[str, Any]) -> dict[str, Any]:
        ...
        if cert.get("expiration_date"):
            try:
                ...
                if 0 < days_until_exp <= 14:
                    forecasts.append(
                        f"HIGH PROBABILITY OUTAGE: Certificate expires in {days_until_exp} days."
                    )
            except ValueError:
                pass
        ...
        for tech in tech_stack:
            if tech.lower() in high_risk_techs:
                forecasts.append(
                    f"ELEVATED RISK TREND: {tech} historically receives critical CVEs frequently. Monitor closely."
                )

        if forecasts:
            logger.warning(
                f"[Prediction Engine] Forecasts generated for {asset.get('target')}: {forecasts}"
            )
