...
class AlertNotifier:
    ...
    async def handle_result(self, result_payload: Dict[str, Any]):
        ...
        for finding in findings:
            severity = finding.get("severity", "").lower() if isinstance(finding, dict) else getattr(finding, "severity", "").lower()
    ...
    async def dispatch_alert(self, finding: Any):
        ...
        target = finding.get("asset_target") if isinstance(finding, dict) else getattr(finding, "asset_target", "Unknown")
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if self.generic_webhook:
                await session.post(self.generic_webhook, json={"message": msg, "severity": severity, "target": target})
