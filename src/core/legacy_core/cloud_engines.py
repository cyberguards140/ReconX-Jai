...
class CloudEngines:
    ...
    def log_public_exposure(project_id, asset_id, finding, severity):
        ...
        if not existing:
            exp = PublicExposure(
                asset_id=asset_id, finding=finding, severity=severity, project_id=project_id
            )
            db.add(exp)
            db.commit()
    ...
    def store_storage_bucket(project_id, provider, name, public=False, object_count=0):
        ...
        existing = (
            ...
            .filter(StorageBucket.project_id == project_id, StorageBucket.name == name)
            .first()
        )
    ...
    def store_iam_entity(project_id, provider, entity_type, name):
        ...
        if not existing:
            entity = IAMEntity(
                provider=provider, entity_type=entity_type, name=name, project_id=project_id
            )
            db.add(entity)
            db.commit()
        db.close()
