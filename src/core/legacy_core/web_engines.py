...
class WebEngines:
    ...
    def store_secret(project_id, category, value, source_url_id=None):
        db = SessionLocal()
        existing = (
            db.query(Secret).filter(Secret.project_id == project_id, Secret.value == value).first()
        )

        if not existing:
            sec = Secret(
                category=category, value=value, source_id=source_url_id, project_id=project_id
            )
            db.add(sec)
            db.commit()

            msg = {"type": "new_secret", "category": category, "project_id": project_id}
            broadcast(msg)
        db.close()
    ...
    def store_technology(project_id, host, name):
        ...
        if not existing:
            tech = Technology(name=name, host=host, project_id=project_id)
            db.add(tech)
            db.commit()

            msg = {"type": "new_technology", "name": name, "project_id": project_id, "host": host}
            broadcast(msg)
        db.close()
    ...
    def create_relationship(source_id, target_id):
        db = SessionLocal()
        rel = (
            db.query(WebRelationship)
            .filter(WebRelationship.source_id == source_id, WebRelationship.target_id == target_id)
            .first()
        )
        if not rel:
