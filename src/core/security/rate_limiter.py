...
class TokenBucketRateLimiter:
    ...
    async def check_rate_limit(self, request: Request):
        ...
        # Check limit
        if bucket["tokens"] < 1:
            raise HTTPException(status_code=429, detail="Too Many Requests. ReconX API rate limit exceeded.")
            
        bucket["tokens"] -= 1

# Global instance for injection
rate_limiter = TokenBucketRateLimiter(capacity=100, refill_rate=2.0) # 100 requests burst, 2 requests/sec refill
