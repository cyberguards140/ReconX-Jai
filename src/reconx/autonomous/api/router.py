...
from reconx.autonomous.recommendations.generator import recommendation_engine
from reconx.autonomous.remediation.planner import remediation_planner
from reconx.autonomous.approvals.workflow import approval_engine
from reconx.autonomous.orchestration.coordinator import coordinator
from reconx.autonomous.governance.auditor import decision_auditor
...
async def trigger_triage(request: EventRequest):
    ...
    # 5. Audit Logging
    decision_auditor.log_decision(plan["plan_id"], "generated_plan", {"risk": risk_score, "status": final_plan["status"]})
...
async def approve_plan(request: ApprovalRequest):
    ...
    if not success:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Plan not found or not pending approval.")

    # Execute the approved plan
    plan_to_execute = {"plan_id": request.plan_id, "status": "approved", "steps": []} # Mocked retrieval
    executed_plan = coordinator.execute_plan(plan_to_execute)
    
    # Audit
    decision_auditor.log_decision(request.plan_id, "approved_and_executed", {"approved_by": user_id})
