...
class AttackSimulationEngine:
    ...
    async def simulate_breach(self, entry_node_id: str, max_depth: int = 4) -> List[Dict[str, Any]]:
        ...
        """
        logger.info(f"[Simulation Engine] Starting breach simulation at {entry_node_id} (Max Depth: {max_depth})")
        ...
        simulated_path = [
            {"step": 1, "tactic": "Initial Access", "node": entry_node_id, "action": "Exploited public CVE"},
            {"step": 2, "tactic": "Credential Access", "node": "EmployeeNode:Admin", "action": "Dumped credentials from compromised asset"},
            {"step": 3, "tactic": "Lateral Movement", "node": "CloudResourceNode:Production_DB", "action": "Authenticated to DB using stolen creds"}
        ]
