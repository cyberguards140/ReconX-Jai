...
class ValidationException(ReconXException):
    def __init__(
        self, message: str = "Validation failed", details: list[dict[str, Any]] | None = None
    ):
...
def setup_exception_handlers(app):
    ...
    from apps.api_gateway.gateway.responses import ErrorDetail, ErrorResponse
    ...
    async def reconx_exception_handler(request: Request, exc: ReconXException):
        details = [
            ErrorDetail(field=d.get("field"), message=d.get("message", str(d))) for d in exc.details
        ]
        error_response = ErrorResponse(
            error_code=exc.error_code, message=exc.message, details=details
        )
