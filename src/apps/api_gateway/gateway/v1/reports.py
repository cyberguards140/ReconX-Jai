...
@router.get("/export")
async def export_report(project_id: str = "default", format: str = "json", db: Session = Depends(get_db)):
    """
...
async def get_compliance_report(framework: str, db: Session = Depends(get_db)):
    ...
    mock_findings = [{"title": "SQL Injection", "asset_target": "api.example.com", "severity": "critical"}]
    mock_exposures = [{"target": "vpn.example.com", "is_critical_exposure": True, "exposure_tags": ["VPN Portal"]}]
