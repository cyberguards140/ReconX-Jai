...
class DashboardAPIMapper:
    ...
    def get_attack_surface_stats(self, project_id: str):
        response = requests.get(f"{self.api_base}/analytics/attack-surface", params={"project_id": project_id})
    ...
    def fetch_assets(self, project_id: str, limit: int = 100):
        response = requests.get(f"{self.api_base}/assets/", params={"project_id": project_id, "limit": limit})
        return response.json() if response.status_code == 200 else {}
