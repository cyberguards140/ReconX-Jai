...
router = APIRouter(
    tags=["Cyber Defense Command Center"], dependencies=[Depends(SecurityMiddleware)]
)
...
async def dispatch_command(request: CommandRequest):
    ...
    global_timeline.log_event(
        event_type="command_dispatched", source="api", payload={"command": request.command_name}
    )
